...
double*** d_initActivationsMatrix(int nl, int inputSize, int outputSize, int nh, int ni){
    double*** activations;
    cudaMalloc(&activations, (nl+2) * sizeof(double**));

    // double*** temp_activations = (double***)malloc((nl+2) * sizeof(double**));
    double** temp_activations = (double**)malloc((nl+2) * sizeof(double*));
    for (int i = 0; i < nl+2; i++) {
        cudaMalloc((void**)&temp_activations[i], ni * sizeof(double*));
        double** temp_layer = (double**)malloc(ni * sizeof(double*));
        for (int j = 0; j < ni; j++){
            int length = (i == 0) ? inputSize : (i == nl+1) ? outputSize : nh;
            cudaMalloc((void**)&temp_layer[j], length * sizeof(double));
        }

        cudaMemcpy(temp_activations[i], temp_layer, ni * sizeof(double*), cudaMemcpyHostToDevice);
        free(temp_layer); 
    }

    cudaMemcpy(activations, temp_activations, (nl+2) * sizeof(double**), cudaMemcpyHostToDevice);
...
double*** d_initWeights(int nl, int inputSize, int outputSize, int nh){
    double ***weights;
    cudaMalloc(&weights, (nl + 1) * sizeof(double**));
    std::random_device rd; 
    std::mt19937 gen(rd());
    double** temp_weights = (double**)malloc((nl + 1) * sizeof(double*));
    for (int i = 0; i < nl + 1; i++) {
        int rows = (i == 0) ? nh : ((i == nl) ? outputSize : nh);
        int cols = (i == 0) ? inputSize : nh;
        cudaMalloc((void**)&temp_weights[i], rows * sizeof(double*));
        std::normal_distribution<double> d(0.0, sqrt(2.0/(cols)));
        double** temp_layer = (double**)malloc(rows * sizeof(double*));
        for (int j = 0; j < rows; j++) {
            cudaMalloc((void**)&temp_layer[j], cols * sizeof(double));

            double* host_layer = (double*)malloc(cols * sizeof(double));
            for (int k = 0; k < cols; k++){
                host_layer[k] = d(gen); 
            }
            cudaMemcpy(temp_layer[j], host_layer, cols * sizeof(double), cudaMemcpyHostToDevice);
            free(host_layer); 
        }
        cudaMemcpy(temp_weights[i], temp_layer, rows * sizeof(double*), cudaMemcpyHostToDevice);
        free(temp_layer); 
    }
    cudaMemcpy(weights, temp_weights, (nl + 1) * sizeof(double**), cudaMemcpyHostToDevice);
    free(temp_weights); 
...
double*** d_initDeltasMatrix(int nl, int outputSize, int nh, int ni){
    double ***deltas;
    cudaMalloc(&deltas, (nl + 1) * sizeof(double**));

    double** temp_deltas = (double**)malloc((nl + 1) * sizeof(double*));
    for (int j = 0; j < nl + 1; j++) {
        cudaMalloc((void**)&temp_deltas[j], ni * sizeof(double*));
        double** temp_layer = (double**)malloc(ni * sizeof(double*));
        for (int i = 0; i < ni; i++) {
            int length = (j == nl) ? outputSize : nh;
            cudaMalloc((void**)&temp_layer[i], length * sizeof(double));
        }
        cudaMemcpy(temp_deltas[j], temp_layer, ni * sizeof(double*), cudaMemcpyHostToDevice);
        free(temp_layer);
    }
    cudaMemcpy(deltas, temp_deltas, (nl + 1) * sizeof(double**), cudaMemcpyHostToDevice);
    free(temp_deltas);

    return deltas;
}

double*** d_initScoresMatrix(int nl, int outputSize, int nh, int ni){
    double ***scores;
    cudaMalloc(&scores, (nl + 2) * sizeof(double**));

    double** temp_scores = (double**)malloc((nl + 2) * sizeof(double*));
    for (int j = 0; j < nl + 2; j++) {
        cudaMalloc((void**)&temp_scores[j], ni * sizeof(double*));
        double** temp_layer = (double**)malloc(ni * sizeof(double*));
        for (int i = 0; i < ni; i++) {
            int length = (j == nl + 1) ? outputSize : nh;
            cudaMalloc((void**)&temp_layer[i], length * sizeof(double));
        }
        cudaMemcpy(temp_scores[j], temp_layer, ni * sizeof(double*), cudaMemcpyHostToDevice);
        free(temp_layer);
    }
    cudaMemcpy(scores, temp_scores, (nl + 2) * sizeof(double**), cudaMemcpyHostToDevice);
    free(temp_scores);
...
double** d_initBiases(int nl, int outputSize, int nh){
    double **biases;
    cudaMalloc(&biases, (nl + 1) * sizeof(double*));

    double** temp_biases = (double**)malloc((nl + 1) * sizeof(double*));
    std::random_device rd; 
    std::mt19937 gen(rd()); 
    ...
    for (int i = 0; i < nl + 1; i++) {
        ...
        // Allocate host memory for biases initialization
        double* host_biases = (double*)malloc(length * sizeof(double));
        for (int j = 0; j < length; j++){
            host_biases[j] = d(gen); 
        }

        // Allocate device memory for biases
        cudaMalloc((void**)&temp_biases[i], length * sizeof(double));
        // Copy initialized biases from host to device
        cudaMemcpy(temp_biases[i], host_biases, length * sizeof(double), cudaMemcpyHostToDevice);

        // Free host memory after copying to device
        free(host_biases);
    }

    // Copy the array of device pointers (biases) from host to device
    cudaMemcpy(biases, temp_biases, (nl + 1) * sizeof(double*), cudaMemcpyHostToDevice);
    free(temp_biases); // Free the temporary host array
...
double** d_initDeltaSum(int nl, int outputSize, int nh){
    double **sumD_host = (double**)malloc((nl + 1) * sizeof(double*)); 
    double **sumD; 
    cudaMalloc(&sumD, (nl + 1) * sizeof(double*)); 

    for (int i = 0; i < nl + 1; i++) {
        int length = (i == nl) ? outputSize : nh;
        cudaMalloc((void**)&sumD_host[i], length * sizeof(double)); 
        cudaMemset(sumD_host[i], 0, length * sizeof(double)); 
    }

    cudaMemcpy(sumD, sumD_host, (nl + 1) * sizeof(double*), cudaMemcpyHostToDevice);
    free(sumD_host);
    return sumD; 
}
...
double*** d_initActivationDeltaSum(int nl, int inputSize, int outputSize, int nh){
    double ***sumAD;
    cudaMalloc(&sumAD, (nl + 1) * sizeof(double**));

    double** temp_layers = (double**)malloc((nl + 1) * sizeof(double*));
    for (int i = 0; i < nl + 1; i++) {
        int rows = (i == 0) ? nh : ((i == nl) ? outputSize : nh);
        int cols = (i == 0) ? inputSize : nh;
        cudaMalloc((void**)&temp_layers[i], rows * sizeof(double*));

        double** temp_row = (double**)malloc(rows * sizeof(double*));
        for (int j = 0; j < rows; j++) {
            cudaMalloc((void**)&temp_row[j], cols * sizeof(double));
            // Initialize the allocated memory to zeros
            cudaMemset(temp_row[j], 0, cols * sizeof(double));
        }
        cudaMemcpy(temp_layers[i], temp_row, rows * sizeof(double*), cudaMemcpyHostToDevice);
        free(temp_row);
    }
    cudaMemcpy(sumAD, temp_layers, (nl + 1) * sizeof(double**), cudaMemcpyHostToDevice);
    free(temp_layers);
...
// relu activation
__device__ void relu(double* results, double* inputs, int length){
    for (int i = 0; i < length; i++){
        results[i] = max(inputs[i], 0.0); 
    }
}
...
__device__ void softmax(double* results, double* inputs, int length){
...
__device__ void vectorAdd(double* vectorA, double* vectorB, double* resultMatrix, int length) {
    for (int i = 0; i < length; ++i) {
...
// element wise addition between two vectors 
__device__ void vectorAddAtomic(double* vectorA, double* resultMatrix, int length) {
    for (int i = 0; i < length; ++i) {
            atomicAdd(&resultMatrix[i], vectorA[i]);
    }
}

// matrix multiplication between a matrix and a vector
__device__ void matrixVectorMultiply(double** matrixA, double* matrixB, double* resultMatrix, int rowsA, int colsA) {
...
__device__ void scalarMatrixMultiplySub(double scalar, double** matrixA, double** resultMatrix, int rows, int cols) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
...
__device__ void scalarVectorMultiplySub(double scalar, double* vectorA, double* resultVector, int length){
    for (int i = 0; i < length; ++i) {
        resultVector[i] -= vectorA[i] * scalar; 
    }
}

__device__ double crossEntropy(const int* a, const double* b, int size){
    double sum = 0; 
...
__device__ void vectorSubtract(double* vectorA, int* vectorB, double* resultMatrix, int length) {
    for (int i = 0; i < length; ++i) {
...
// does element-wise multiplication of two vectors 
__device__ void vectorMultiply(double* vectorA, double* vectorB, double* resultMatrix, int length) {
    for (int i = 0; i < length; ++i) {
        resultMatrix[i] = vectorA[i] * vectorB[i];
    }
}

// transposes vector A, multiplies by vectorB, and subtracts from result
__device__ void addVectorMultiplyTranspose(double* matrixA, double* matrixB, double** resultMatrix, int length, int width) {
    for (int i = 0; i < length; ++i) {
        for (int j = 0; j < width; ++j){
            resultMatrix[i][j] += matrixA[i]*matrixB[j]; 
        }
    }
}

// transposes vector A, multiplies by vectorB, and subtracts from result
__device__ void addVectorMultiplyTransposeAtomic(double* matrixA, double* matrixB, double** resultMatrix, int length, int width) {
    for (int i = 0; i < length; ++i) {
        for (int j = 0; j < width; ++j){
            atomicAdd(&resultMatrix[i][j], matrixA[i]*matrixB[j]); 
        }
    }
}

// tranposes the matrix and multiplies it by a vector
__device__ void matrixVectorMultiplyTranspose(double** matrixA, double* matrixB, double* resultMatrix, int rowsA, int colsA) {
    for (int i = 0; i < colsA; ++i) {
        resultMatrix[i] = 0;
        for (int j = 0; j < rowsA; ++j) {
            resultMatrix[i] += matrixA[j][i] * matrixB[j]; 
        }
    }
}
